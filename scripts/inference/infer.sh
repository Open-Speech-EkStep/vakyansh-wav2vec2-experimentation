...
if [ "${w2l_decoder_viterbi}" = 1 ]; then
  ...
  --nbest 1 --path ${checkpoint_path} --gen-subset ${subset} --results-path ${result_path}/${inference_data_name} --w2l-decoder viterbi \
  ...
  python ${wav2vec_repo_path}examples/speech_recognition/infer.py ${data_path} --task audio_pretraining \
  --nbest 1 --path ${checkpoint_path} --gen-subset ${subset} --results-path ${result_path}/${inference_data_name}_kenlm_${beam} --w2l-decoder kenlm --lm-model ${lm_model_path}\
